      ...
      upsert            = function(db_conn = NULL, db_type = NULL, table_name = NULL, data = NULL, id_col = NULL) {
         ...
         if (nrow(data) >= chunk_size) {
            ...
            for (i in seq_len(length(data))) {
               ...
               suppress_warnings(DBI::dbAppendTable(db_conn, table_sql, data[[i]], row.names = NA), 'column')
               pb$tick(chunk_bytes)
            }
            cat("\n")
         } else {
            dbxDelete(db_conn, table_space, data %>% select(any_of(id_col)), batch_size = 1000)
            suppress_warnings(DBI::dbAppendTable(db_conn, table_sql, data, row.names = NA), 'column')
         }
      },
