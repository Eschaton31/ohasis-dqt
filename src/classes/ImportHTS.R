ImportHTS <- R6Class(
   ...
   public = list(
      ...
      read       = function(sheets) {
         if (missing(sheets)) {
            sheets <- excel_sheets(self$file)
         }

         logsheet <- list()
         for (sheet in sheets) {
            test <- read_excel(self$file, sheet, n_max = 3, .name_repair = "unique_quiet")

            if (ncol(test) == 121 | ncol(test) == 79) {
               logsheet[[sheet]] <- read_excel(self$file, sheet, skip = 2, .name_repair = "unique_quiet") %>%
...
try <- ImportHTS$new("W:/Users/johnb/Downloads/protects-upscale-june2024/AMOS_2024_JUNE.xlsx")
try$read()
