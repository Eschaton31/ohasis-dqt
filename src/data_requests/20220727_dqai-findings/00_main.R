				...
				addConditionalFormatRule = list(
					rule = list(
						...
						booleanRule = list(
							condition = list(
								type   = "CUSTOM_FORMULA",
								values = list(userEnteredValue = r"(=AND($P1=$V1,$P1<>""))")
							),
							format    = list(
...
dqai$gsheet   <- "1cSnJ9_pN-_1j-ZnkCzJPAE6J8vKGSSir2iK-Tx-9hxs"
dqai$reg_art  <- ohasis$get_data("harp_tx-reg", "2022", "06") %>% read_dta()
dqai$dir      <- "C:/Users/Administrator/Documents/DQAI 2022/DRH"
dqai$dta_list <- list()
for (file in list.files(dqai$dir, "*.csv", full.names = TRUE)) {
	...
	dqai$dta_list[[fname]] <- read_csv(file, col_types = "c") %>%
		...
		mutate(
			`DRH Patient Code`           = NA_character_,
			`DRH UIC`                    = NA_character_,
			`DRH Outcome`                = NA_character_,
			`DRH Facility (referred to)` = NA_character_,
			`DRH Regimen`                = NA_character_,
			`DRH Latest Visit`           = NA_character_,
			`DRH Latest Next Pick-up`    = NA_character_,
			`DRH No. of Pills Dispensed` = NA_character_,
			outcome                      = fname,
...
# write_xlsx(dqai$dta_list, "C:/Users/Administrator/Documents/DQAI 2022/DRH/zcm_masterlist_2022-05.xlsx")

dqai$eb <- bind_rows(dqai$dta_list)

dqai$faci <- bind_rows(dqai$sheet_list) %>%
   rename_all(~stri_replace_all_fixed(., "DRH", "HUB")) %>%
   filter(
	  nchar(`OHASIS ID`) <= 18,
