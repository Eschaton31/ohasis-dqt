dqai        <- new.env()
dqai$bmc    <- list()
dqai$bmc$ss <- "1EmacCXifEOmSdT5iiFwIxjdtdZafEjK-5EhWyOmuyhI"
...
dqai$bmc$data$hub       <- read_sheet(dqai$bmc$ss, "BMC Masterlist 2022-06")
dqai$bmc$data$eb        <- read_sheet(dqai$bmc$ss, "EB Conso")
dqai$bmc$data$dashboard <- bind_rows(
   read_sheet(dqai$bmc$ss, "onart", col_types = "c"),
   read_sheet(dqai$bmc$ss, "ltfu", col_types = "c"),
   read_sheet(dqai$bmc$ss, "dead", col_types = "c"),
   read_sheet(dqai$bmc$ss, "transout", col_types = "c")
)

dqai$bmc$match$dashboard <- dqai$bmc$data$dashboard %>%
   mutate(
      final_px          = if_else(
         condition = !is.na(`BMC Patient Code`),
         true      = `BMC Patient Code`,
         false     = `Patient Code`
      ),
      validated_outcome = tolower(`BMC Outcome`),
...
dqai$bmc$match$eb <- dqai$bmc$data$eb %>%
   filter(
...
dqai$bmc$match$hub <- dqai$bmc$data$hub %>%
   rename(
      outcome = Status
   ) %>%
   mutate(
      final_px = `Patient Code`,
   ) %>%
   filter(
      # duplicate_drop == FALSE,
      outcome %in% c("onart", "transout", "ltfu", "dead", "stopped"),
      # !is.na(latest_regimen)
   )

dqai$bmc$match$final <- dqai$bmc$match$eb %>%
   mutate(final_px = toupper(final_px)) %>%
   full_join(
      y  = dqai$bmc$match$dashboard %>%
         mutate(final_px = toupper(final_px)) %>%
         select(
            final_px,
            validated_outcome
   ...
   mutate(
      final_outcome = case_when(
         validated_outcome == "stopped" ~ "stopped",
         faci_outcome == "stopped" ~ "stopped",
         validated_outcome == "transient" ~ "transient",
...
dqai$bmc$final$hub_ml <- dqai$bmc$data$hub_raw %>%
   left_join(
      y  = dqai$bmc$data$hub %>%
         select(
            num,
