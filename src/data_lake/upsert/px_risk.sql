SELECT exp_data.REC_ID,
       ...
       MAX(IF(exp_data.EXPOSURE = 'HIV_MOTHER', DATE(exp_data.DATE_LAST_EXPOSE), NULL))    AS RISK_HIV_MOTHER_DATE,

       -- sex w/ males
       MAX(IF(exp_data.EXPOSURE = 'SEX_M', exp_data.IS_EXPOSED, NULL))                     AS RISK_SEX_M,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M', DATE(exp_data.DATE_LAST_EXPOSE), NULL))         AS RISK_SEX_M_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M_AV', exp_data.IS_EXPOSED, NULL))                  AS RISK_SEX_M_AV,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M_AV', DATE(exp_data.DATE_LAST_EXPOSE), NULL))      AS RISK_SEX_M_AV_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M_NOCONDOM', exp_data.IS_EXPOSED, NULL))            AS RISK_SEX_M_AV_NOCONDOM,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M_NOCONDOM', DATE(exp_data.DATE_LAST_EXPOSE),
              NULL))                                                                       AS RISK_SEX_M_AV_NOCONDOM_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M_AV_NOCONDOM', exp_data.IS_EXPOSED,
              NULL))                                                                       AS RISK_SEX_M_AV_NOCONDOM,
       MAX(IF(exp_data.EXPOSURE = 'SEX_M_AV_NOCONDOM', DATE(exp_data.DATE_LAST_EXPOSE),
              NULL))                                                                       AS RISK_SEX_M_AV_NOCONDOM_DATE,

       -- sex w/ females
       MAX(IF(exp_data.EXPOSURE = 'SEX_F', exp_data.IS_EXPOSED, NULL))                     AS RISK_SEX_F,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F', DATE(exp_data.DATE_LAST_EXPOSE), NULL))         AS RISK_SEX_F_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F_AV', exp_data.IS_EXPOSED, NULL))                  AS RISK_SEX_F_AV,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F_AV', DATE(exp_data.DATE_LAST_EXPOSE), NULL))      AS RISK_SEX_F_AV_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F_NOCONDOM', exp_data.IS_EXPOSED, NULL))            AS RISK_SEX_F_AV_NOCONDOM,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F_NOCONDOM', DATE(exp_data.DATE_LAST_EXPOSE),
              NULL))                                                                       AS RISK_SEX_F_AV_NOCONDOM_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F_AV_NOCONDOM', exp_data.IS_EXPOSED,
              NULL))                                                                       AS RISK_SEX_F_AV_NOCONDOM,
       MAX(IF(exp_data.EXPOSURE = 'SEX_F_AV_NOCONDOM', DATE(exp_data.DATE_LAST_EXPOSE),
              NULL))                                                                       AS RISK_SEX_F_AV_NOCONDOM_DATE,
       ...
       MAX(IF(exp_data.EXPOSURE = 'SEX_EVER', DATE(exp_data.DATE_LAST_EXPOSE), NULL))      AS RISK_SEX_EVER_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_ORAL_ANAL', exp_data.IS_EXPOSED, NULL))             AS RISK_SEX_ORAL_ANAL,
       MAX(IF(exp_data.EXPOSURE = 'SEX_ORAL_ANAL', DATE(exp_data.DATE_LAST_EXPOSE), NULL)) AS RISK_SEX_ORAL_ANAL_DATE,

       -- transactional sex
       MAX(IF(exp_data.EXPOSURE = 'SEX_PAYING', exp_data.IS_EXPOSED, NULL))                AS RISK_SEX_PAYING,
       MAX(IF(exp_data.EXPOSURE = 'SEX_PAYING', DATE(exp_data.DATE_LAST_EXPOSE), NULL))    AS RISK_SEX_PAYING_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_PAYMENT', exp_data.IS_EXPOSED, NULL))               AS RISK_SEX_PAYMENT,
       MAX(IF(exp_data.EXPOSURE = 'SEX_PAYMENT', DATE(exp_data.DATE_LAST_EXPOSE), NULL))   AS RISK_SEX_PAYMENT_DATE,
       MAX(IF(exp_data.EXPOSURE = 'SEX_TRANSACT', exp_data.IS_EXPOSED, NULL))              AS RISK_SEX_TRANSACT,
       MAX(IF(exp_data.EXPOSURE = 'SEX_TRANSACT', DATE(exp_data.DATE_LAST_EXPOSE), NULL))  AS RISK_SEX_TRANSACT_DATE,

       -- hiv exposure
       MAX(IF(exp_data.EXPOSURE = 'SEX_HIV', exp_data.IS_EXPOSED, NULL))                   AS RISK_SEX_HIV,
       MAX(IF(exp_data.EXPOSURE = 'SEX_HIV', DATE(exp_data.DATE_LAST_EXPOSE), NULL))       AS RISK_SEX_HIV_DATE,
       MAX(IF(exp_data.EXPOSURE = 'HIV_UNKNOWN', exp_data.IS_EXPOSED, NULL))               AS RISK_HIV_UNKNOWN,
       MAX(IF(exp_data.EXPOSURE = 'HIV_UNKNOWN', DATE(exp_data.DATE_LAST_EXPOSE), NULL))   AS RISK_HIV_UNKNOWN_DATE,
       ...
       MAX(IF(exp_data.EXPOSURE = 'SEX_DRUGS', DATE(exp_data.DATE_LAST_EXPOSE), NULL))     AS RISK_SEX_DRUGS_DATE,
       MAX(IF(exp_data.EXPOSURE = 'DRUG_INJECT', exp_data.IS_EXPOSED, NULL))               AS RISK_DRUG_INJECT,
       MAX(IF(exp_data.EXPOSURE = 'DRUG_INJECT', DATE(exp_data.DATE_LAST_EXPOSE), NULL))   AS RISK_DRUG_INJECT_DATE,
       MAX(IF(exp_data.EXPOSURE = 'ILLICIT_DRUGS', exp_data.IS_EXPOSED, NULL))             AS RISK_ILLICIT_DRUGS,
       MAX(IF(exp_data.EXPOSURE = 'ILLICIT_DRUGS', DATE(exp_data.DATE_LAST_EXPOSE), NULL)) AS RISK_ILLICIT_DRUGS_DATE,
       ...
       MAX(IF(exp_data.EXPOSURE = 'OCCUPATION', DATE(exp_data.DATE_LAST_EXPOSE), NULL))    AS RISK_OCCUPATION_DATE,
       MAX(IF(exp_data.EXPOSURE = 'PEP', exp_data.IS_EXPOSED, NULL))                       AS RISK_PEP,
       MAX(IF(exp_data.EXPOSURE = 'PEP', DATE(exp_data.DATE_LAST_EXPOSE), NULL))           AS RISK_PEP_DATE,
       MAX(IF(exp_data.EXPOSURE = 'STI', exp_data.IS_EXPOSED, NULL))                       AS RISK_STI,
       MAX(IF(exp_data.EXPOSURE = 'STI', DATE(exp_data.DATE_LAST_EXPOSE), NULL))           AS RISK_STI_DATE,
       MAX(IF(exp_data.EXPOSURE = 'TATTOO', exp_data.IS_EXPOSED, NULL))                    AS RISK_TATTOO,
       MAX(IF(exp_data.EXPOSURE = 'TATTOO', DATE(exp_data.DATE_LAST_EXPOSE), NULL))        AS RISK_TATTOO_DATE
