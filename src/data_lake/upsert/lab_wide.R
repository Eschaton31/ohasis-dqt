   ...
   object   <- object %>%
      ...
      inner_join(
         y  = tbl(db_conn, dbplyr::in_schema("ohasis_interim", "px_record")) %>%
            ...
            select(
               REC_ID,
               LAB_HEMOG_DATE     = DATE_HEMOG,
               LAB_HEMOG_RESULT   = RESULT_HEMOG,
               LAB_VIRAL_DATE     = DATE_VIRAL,
               LAB_VIRAL_RESULT   = RESULT_VIRAL,
               LAB_CD4_DATE       = DATE_CD4,
               LAB_CD4_RESULT     = RESULT_CD4,
               LAB_CREA_DATE      = DATE_CREA,
               LAB_CREA_RESULT    = RESULT_CREA,
               LAB_CREA_CLEARANCE = RESULT_OTHER_CREA,
               LAB_HBSAG_DATE     = DATE_HBSAG,
               LAB_HBSAG_RESULT   = RESULT_HBSAG,
               LAB_XRAY_DATE      = DATE_XRAY,
               LAB_XRAY_RESULT    = RESULT_XRAY,
               LAB_XPERT_DATE     = DATE_XPERT,
               LAB_XPERT_RESULT   = RESULT_XPERT,
               LAB_DSSM_DATE      = DATE_DSSM,
               LAB_DSSM_RESULT    = RESULT_DSSM,
               LAB_HIVDR_DATE     = DATE_HIVDR,
               LAB_HIVDR_RESULT   = RESULT_HIVDR,
               LAB_SYPH_DATE      = DATE_SYPH,
               LAB_SYPH_RESULT    = RESULT_SYPH,
               LAB_SYPH_TITER     = RESULT_OTHER_SYPH,
            ) %>%
            collect(),
         by = "REC_ID"
      )
