...
aiha <- list(
   path    = "C:/Users/Administrator/Downloads/aiha_offline_2023",
   convert = list(
...
report_files <- list.files(aiha$path, ".xls*", full.names = TRUE)
...
aiha$hts <- raw %>%
   ...
   filter(as.Date(RECORD_DATE) %within% interval("2023-04-01", "2023-11-24"))
...
aiha$prev_upload %>%
   ...
   filter(PATIENT_ID == "20231130990005G142") %>%
   write_clip()
