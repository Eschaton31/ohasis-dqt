...
dqai$forms$form_art_bc <- dbTable(
   lw_conn,
   "ohasis_warehouse",
   "form_art_bc",
   cols      = c(
   ...
   where     = "ART_RECORD = 'ART'",
   raw_where = TRUE
)

dqai$forms$id_registry <- dbTable(
   lw_conn,
   "ohasis_warehouse",
   "id_registry",
...
dqai$harp$dx <- ohasis$get_data("harp_dx", "2022", "06") %>%
   read_dta() %>%
   zap_missing()

dqai$harp$tx$reg <- ohasis$get_data("harp_tx-reg", "2022", "06") %>%
   read_dta() %>%
   zap_missing() %>%
   select(-CENTRAL_ID) %>%
   left_join(
...
dqai$harp$tx$outcome <- ohasis$get_data("harp_tx-outcome", "2022", "06") %>%
   read_dta() %>%
   zap_missing()

##  Consolidate data -----------------------------------------------------------

dqai$coverage$next_date <- ohasis$get_date_ref("next", "2022", "06")
