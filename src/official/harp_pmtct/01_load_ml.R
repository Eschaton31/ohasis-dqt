...
for (xlsx in xlsx_files) {
   ...
   if (hub == "vcm") {
      ...
      if (!is.null(password) && !is.na(password)) {
         wb <- loadWorkbook(xlsx, password = password)
      } else {
   ...
   if (length(sheets) > 1) {
      ...
      if (hub == "bic") {
         ...
         tmp_list                    <- list()
         tmp_list[[glue("{hub}_1")]] <- readWorksheet(
            wb,
            sheets[1],
         ...
         tmp_list[[glue("{hub}_2")]] <- readWorksheet(
            wb,
            sheets[1],
            colTypes = XLC$DATA_TYPE.STRING,
            startRow = 4,
         ...
         for (i in seq_len(length(sheets))) {
            tmp_list[[glue("{hub}_{i}_1")]] <- readWorksheet(
               wb,
               sheets[i],
               colTypes = XLC$DATA_TYPE.STRING,
               startRow = 3,
            ...
            tmp_list[[glue("{hub}_{i}_2")]] <- readWorksheet(
               wb,
               sheets[i],
               colTypes = XLC$DATA_TYPE.STRING,
               startRow = 3,
         ...
         tmp_list <- list()
         for (i in seq_len(length(sheets)))
            tmp_list[[glue("{hub}_{i}")]] <- readWorksheet(
               wb,
               sheets[i],
               colTypes = XLC$DATA_TYPE.STRING,
               startRow = start_row,
