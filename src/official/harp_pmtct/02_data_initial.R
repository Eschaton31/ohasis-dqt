...
data_forms <- tbl(lw_conn, dbplyr::in_schema("ohasis_warehouse", "form_art_bc")) %>%
   select(
      PATIENT_ID,
      VISIT_DATE,
      LAB_VIRAL_DATE,
   ...
   # keep only dates w/in set period
   filter(
      !is.na(LAB_VIRAL_DATE) | !is.na(LAB_VIRAL_RESULT),
      VISIT_DATE >= start_vl | LAB_VIRAL_DATE >= start_vl
   ) %>%
...
vl_data <- data_forms %>%
   ...
   bind_rows(
      read_dta("H:/_R/library/hiv_vl/data/20220510_vl_ml_ever.dta") %>%
         filter(PATIENT_ID != "") %>%
         mutate_if(
            .predicate = is.character,
            ~if_else(. == "", NA_character_, .)
...
onart_prev <- read_dta("H:/_R/library/hiv_tx/data/20220207_onart_2021-12.dta") %>%
   # convert Stata string missing data to NAs
   mutate_if(
      .predicate = is.character,
      ~if_else(. == '', NA_character_, .)
...
onart_vl <- nhsss$harp_tx$official$new_reg %>%
   ...
   left_join(
      y  = onart_prev %>% select(CENTRAL_ID, prev_vldate = vl_date, prev_vlresult = vl_result, prev_restag = res_tag, prev_vlp12m = vlp12m),
      by = "CENTRAL_ID"
   ) %>%
   mutate(
