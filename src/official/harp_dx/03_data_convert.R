...
if (update == "1") {
   ...
   # special checks
   nhsss$harp_dx$converted$check[["perinatal"]] <- nhsss$harp_dx$converted$data %>%
      filter(
         transmit == "PERINATAL"
      ) %>%
      select(
   ...
   for (var in vars) {
      ...
      nhsss$harp_dx$converted$check$tabstat <- df %>%
         summarise(
         ...
         bind_rows(nhsss$harp_dx$converted$check$tabstat)
   }
}
...
gdrive_validation(nhsss$harp_dx, "converted", ohasis$ym)
