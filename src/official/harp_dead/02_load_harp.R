...
old_dta       <- ohasis$get_data("harp_dead", ohasis$prev_yr, ohasis$prev_mo)
old_corr      <- nhsss$harp_dead$corr$harp_dead_old
old_tblname   <- "harp_dead_old"
...
reload <- input(
   prompt  = "How do you want to load the previous HARP Dead dataset?",
...
if (reload == "1") {
   ...
   .log_info("Updating `harp_dead_old`.")
   # delete existing data, full refresh always
   if (dbExistsTable(lw_conn, old_tblspace))
      dbExecute(lw_conn, "DROP TABLE `ohasis_warehouse`.`harp_dead_old`;")

   # upload info
   ohasis$upsert(lw_conn, "warehouse", old_tblname, old_dataset, "PATIENT_ID")

   # TODO: add logs insert for harp_dead

   # assign to global environment
   nhsss$harp_dead$official$old <- old_dataset
   rm(old_dataset)

   slackr_msg(
      paste0(">`harp_dead_old` updated with the ", ohasis$prev_yr, ".", ohasis$prev_mo, " version."),
      mrkdwn = "true"
   )
}
...
if (reload == "2") {
   ...
   nhsss$harp_dead$official$old <- tbl(lw_conn, old_tblschema) %>%
      select(-CENTRAL_ID) %>%
      left_join(
         y  = tbl(lw_conn, oh_id_schema) %>%
            select(CENTRAL_ID, PATIENT_ID),
