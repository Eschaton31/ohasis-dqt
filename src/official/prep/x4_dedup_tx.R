...
.init <- function() {
   ...
   local(envir = p, {
      tx   <- ohasis$get_data("harp_tx", ohasis$yr, ohasis$mo) %>%
         read_dta(
            col_select = c(
               PATIENT_ID,
               labcode,
      ...
      check   <- append(check, prep_merge(prep, tx))
      rm(tx, prep, reclink)
   })

   local(envir = .GlobalEnv, flow_validation(nhsss$prep, "dedup_tx", ohasis$ym))
}
