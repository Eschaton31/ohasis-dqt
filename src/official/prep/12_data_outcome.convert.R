...
nhsss$prep$outcome.converted$data <- nhsss$prep$outcome.initial$data %>%
   distinct(prep_id, .keep_all = TRUE) %>%
   # prepare screening taggings
   mutate(
      # ars screening
      ars_screen  = case_when(
         StrLeft(ARS_SX_NONE, 1) == "1" ~ 0,
         StrLeft(PRE_INIT_NO_ARS, 1) == "1" ~ 0,
         StrLeft(ARS_SX_FEVER, 1) == "1" ~ 1,
         StrLeft(ARS_SX_DIARRHEA, 1) == "1" ~ 1,
         StrLeft(ARS_SX_SORE_THROAT, 1) == "1" ~ 1,
         StrLeft(ARS_SX_SWOLLEN_LYMPH, 1) == "1" ~ 1,
         StrLeft(ARS_SX_SWOLLEN_TONSILS, 1) == "1" ~ 1,
         StrLeft(ARS_SX_RASH, 1) == "1" ~ 1,
         StrLeft(ARS_SX_MUSCLE_PAINS, 1) == "1" ~ 1,
         StrLeft(ARS_SX_OTHER, 1) == "1" ~ 1,
         TRUE ~ 9999
      ),
      ...
      dispensed   = case_when(
         ...
         !is.na(PREP_STATUS) ~ as.numeric(StrLeft(PREP_STATUS, 1)),
         !is.na(PREP_CONTINUED) ~ as.numeric(StrLeft(PREP_CONTINUED, 1)),
         TRUE ~ 9
      ),
      prep_nr     = case_when(
         StrLeft(PRE_INIT_HIV_NR, 1) == "1" ~ 1,
         !is.na(PREP_HIV_DATE) ~ 1,
         TRUE ~ 0
      ),
      prep_weight = case_when(
         StrLeft(PRE_INIT_WEIGHT, 1) == "1" ~ 1,
         floor(WEIGHT) >= 35 ~ 1,
         TRUE ~ 0
      ),
      prep_behave = case_when(
         StrLeft(ELIGIBLE_BEHAVIOR, 1) == "1" ~ 1,
         StrLeft(PREP_REQUESTED, 1) == "1" ~ 1,
         TRUE ~ 0
      ),
      eligible    = case_when(
         dispensed == 1 ~ 1,
         StrLeft(ELIGIBLE_PREP, 1) == "1" ~ 1,
         prep_nr == 1 &
            prep_weight == 1 &
            ars_screen == 0 &
            prep_behave == 1 ~ 1,
      ...
      # prep_on     = case_when(
      #    dispensed == 1 ~ "on prep",
      #    StrLeft(PREP_STATUS, 1) == "0" ~ "refused",
      #    StrLeft(PREP_CONTINUED, 1) == "0" ~ "discontinued",
      #    dispensed == 0 & eligible == 1 ~ "not on prep",
      #    eligible == 1 & dispensed == 9 ~ "eligible",
      #    prep_screen == "yes" ~ "screened",
      #    prep_screen == "" ~ "not screened",
      # )
   ) %>%
   # get mortality data
   left_join(
   ...
   mutate(
      ...
      curr_age            = case_when(
         ...
         is.na(birthdate) & use_db == 1 & !is.na(age) ~ age + (year(LATEST_VISIT) - year(artstprep_date)),
         ...
         is.na(birthdate) & use_db == 0 & !is.na(age) ~ age + (year(prev_ffup) - year(artstprep_date)),
         is.na(birthdate) & use_db == 0 & is.na(age) ~ prev_age + (year(prev_ffup) - year(artstprep_date)),
      ) %>% floor(),
...
nhsss$prep$outcome.converted$data %<>%
   ...
   mutate(
      curr_line = case_when(
         prep_reg1 == "tdf 3tc efv" ~ 1,
         prep_reg1 == "tdf 3tc nvp" ~ 1,
         prep_reg1 == "azt 3tc efv" ~ 1,
         prep_reg1 == "azt 3tc nvp" ~ 1,
         prep_reg1 == "d4t 3tc efv" ~ 1,
         prep_reg1 == "d4t 3tc nvp" ~ 1,
         prep_reg1 == "tdf 3tc dtg" ~ 1,
         prep_reg1 == "3tc nvp abc" ~ 1,
         prep_reg1 == "3tc efv abc" ~ 1,
         prep_reg1 == "3tc abc ril" ~ 1,
         prep_reg1 == "tdf 3tc ril" ~ 1,
         prep_reg1 == "azt 3tc ril" ~ 1,
         prep_reg1 == "azt 3tc lpv/r" ~ 2,
         prep_reg1 == "tdf 3tc lpv/r" ~ 2,
         prep_reg1 == "d4t 3tc lpv/r" ~ 2,
         prep_reg1 == "d4t 3tc idv" ~ 2,
         prep_reg1 == "azt 3tc idv" ~ 2,
         prep_reg1 == "tdf 3tc idv" ~ 2,
         prep_reg1 == "3tc lpv/r abc" ~ 2,
         !is.na(prep_reg1) ~ 3
      ),
   ) %>%
