...
icap$linelist$hts_tst_verify <- bind_rows(icap$forms$form_cfbs, icap$forms$form_hts) %>%
   ...
   mutate(
      ...
      # disaggregations
      `DISAG 2`       = case_when(
         confirm_date >= as.Date(icap$coverage$min) ~ "CBS Reactive",
         ref_report < as.Date(icap$coverage$min) ~ "Known Pos",
         TRUE ~ "CBS Reactive"
      ),
...
icap$linelist$hts_tst_verify <- ohasis$get_faci(
   icap$linelist$hts_tst_verify,
