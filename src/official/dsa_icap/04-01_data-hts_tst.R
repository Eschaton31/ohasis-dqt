...
icap$linelist$hts_tst <- bind_rows(icap$forms$form_hts, icap$forms$form_a) %>%
   filter(
   ...
   filter(
      (LATEST_CONFIRM_DATE >= as.Date(icap$coverage$min) & LATEST_CONFIRM_DATE <= as.Date(icap$coverage$max))
   ) %>%
   ...
   full_join(
      y  = bind_rows(icap$forms$form_hts, icap$forms$form_a) %>%
         ...
         filter(
            (LATEST_TEST_DATE >= as.Date(icap$coverage$min) & LATEST_TEST_DATE <= as.Date(icap$coverage$max))
         ) %>%
   ...
   full_join(
      y  = bind_rows(icap$forms$form_hts, icap$forms$form_cfbs) %>%
         ...
         filter(
            (LATEST_CFBS_DATE >= as.Date(icap$coverage$min) & LATEST_CFBS_DATE <= as.Date(icap$coverage$max))
         ) %>%
   ...
   full_join(
      y  = icap$forms$form_hts %>%
         ...
         filter(
            (LATEST_ST_DATE >= as.Date(icap$coverage$min) & LATEST_ST_DATE <= as.Date(icap$coverage$max))
         ) %>%
   ...
   mutate(
      ...
      # old confirmed
      old_dx            = case_when(
         confirm_date >= as.Date(icap$coverage$min) ~ 0,
         ref_report < as.Date(icap$coverage$min) ~ 1,
         TRUE ~ 0
      ),
...
icap$linelist$hts_tst <- ohasis$get_faci(
   icap$linelist$hts_tst,
