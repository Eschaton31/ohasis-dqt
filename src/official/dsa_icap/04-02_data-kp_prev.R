...
icap$linelist$kp_prev <- icap$linelist$hts_tst %>%
   mutate(
      SCREEN_AGREED = "1_Yes"
   ) %>%
   select(
   ...
   bind_rows(
      icap$forms$form_hts %>%
         ...
         bind_rows(icap$forms$form_cfbs) %>%
         mutate(
         ...
         filter(
            (RECORD_DATE >= as.Date(icap$coverage$min) & RECORD_DATE <= as.Date(icap$coverage$max))
         ) %>%
...
icap$linelist$kp_prev <- ohasis$get_faci(
   icap$linelist$kp_prev,
