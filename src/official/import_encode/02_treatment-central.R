...
import          <- new.env()
import$drive_ss <- as_id("18hh6GZzjnNBidMg9sxOworj2IhbwTUak")
import$STAFF    <- read_sheet(as_id("1BRohoSaBE73zwRMXQNcWeRf5rC2OcePS64A67ODfxXI"))
import$ym       <- "2023.10"

##  download -------------------------------------------------------------------

dir_all   <- as_id(drive_ls(import$drive_ss, pattern = import$ym)$id)
dir_month <- as_id(drive_ls(dir_all, pattern = "TREATMENT")$id)
dir_local <- file.path(getwd(), "data", "treatment-central")
check_dir(dir_local)

files_month <- drive_ls(dir_month, pattern = import$ym)
apply(files_month, 1, function(row) {
   ss    <- as_id(row$id)
   name  <- row$name
   email <- str_squish(substr(name, 9, stri_locate_first_fixed(name, ".com") + 4))
   file  <- file.path(dir_local, stri_c(email, ".xlsx"))
   # sheets <- c("FORMS", "DISPENSE", "DISCONTINUE", "ref_faci", "ref_addr", "ref_meds")
   log_info(green(email))

   drive_download(ss, file, type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", overwrite = TRUE)
   # data <- lapply(sheets, read_sheet, ss = row$id, col_types = "c", .name_repair = "unique_quiet")
})
files_local <- list.files(dir_local, full.names = TRUE)
forms       <- lapply(files_local, read_excel, sheet = "FORMS", col_types = "text", .name_repair = "unique_quiet")
...
local(envir = import, {
   ...
   download_ei <- function() {
      ...
      encode_mo     <- as_id(drive_ls(import$drive_ss, pattern = ym)$id)
      encode_surv   <- as_id(drive_ls(encode_mo, pattern = "TREATMENT")$id)
      encode_sheets <- drive_ls(encode_surv, pattern = ym)
      data          <- list()
   ...
   tables_long <- function(standard, encoded) {
      ...
      tables$px_medicine %>% tabstat(days)
      tables$px_medicine %>% tabstat(DISP_DATE)
      ...
      tables$px_medicine_disc <- standard %>%
         ...
         inner_join(
            y          = encoded$DISCONTINUE %>%
               mutate(
                  encoder = stri_replace_all_fixed(encoder, glue("{ym}_"), ""),
               ),
