...
data_ml %<>%
   ...
   mutate(
      ...
      vl_date_2     = case_when(
         ...
         stri_detect_fixed(vl_date, "/") & hub == "tch" ~ as.Date(str_left(vl_date, 10), "%d/%m/%Y"),
         stri_detect_fixed(vl_date, "/") & hub == "dcs" ~ as.Date(str_left(vl_date, 10), "%d/%m/%Y"),
         stri_detect_fixed(vl_date, "(") & hub == "ccc" ~ as.Date(substr(vl_result, 2, 11), "%m/%d/%Y"),
         stri_detect_fixed(vl_date, "-") & StrIsNumeric(str_left(vl_date, 4)) ~ as.Date(str_left(vl_date, 10), "%Y-%m-%d"),
         stri_detect_fixed(vl_date, "/") & StrIsNumeric(str_left(vl_date, 4)) ~ as.Date(str_left(vl_date, 10), "%Y/%m/%d"),
         first_slash %in% c(2, 3) ~ as.Date(str_left(vl_date, 10), "%m/%d/%Y"),
         first_dash %in% c(2, 3) ~ as.Date(str_left(vl_date, 10), "%m-%d-%Y"),
         first_dot %in% c(2, 3) ~ as.Date(str_left(vl_date, 10), "%m.%d.%Y"),
