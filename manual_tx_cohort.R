...
dx      <- QB$new(lw_conn)$
   select(idnum,
          ...
          artstart_date,
          dead)$
   from(stri_c("harp_full.reg_", yr, stri_pad_left(mo, 2, '0')))$
   get()
tx_reg  <- QB$new(lw_conn)$
   select(patient_id,
          art_id,
          mort_id,
          idnum,
          ...
          baseline_cd4_date,
          baseline_cd4_result)$
   from(stri_c("harp_tx.reg_", yr, stri_pad_left(mo, 2, '0')))$
   get()
tx_out  <- QB$new(lw_conn)$
   select(art_id,
          regimen,
          sex,
          latest_regimen,
          ...
          vlp12m,
          curr_age)$
   from(stri_c("harp_tx.outcome_", yr, stri_pad_left(mo, 2, '0')))$
   get()
dbDisconnect(lw_conn)

generate_tx_cohort <- function(yr, mo, faci) {
   con <- connect('ohasis-live')
   hub <- QB$new(con)
   hub$selectRaw("coalesce(id.central_id, rec.patient_id) AS central_id")
   hub$from("ohasis.px_record AS rec")
   hub$leftJoin("ohasis.registry AS id", "rec.patient_id", "=", "id.patient_id")
   hub$distinct()
   hub$whereIn("faci_id", (ohasis$ref_faci %>% filter(faci_code == toupper(faci)))$faci_id)
   hub$where("module", "3")
   cohort <- hub$get()
   dbDisconnect(con)
   ...
   export <- cohort %>%
      ...
      mutate(
         refer_status = case_when(
            ART_FACI %in% ((ohasis$ref_faci %>% filter(faci_code == toupper(faci)))$faci_id) ~ "Last visited here",
   ...
   write_xlsx(export, glue("C:/Users/Bene-G16/Downloads/{date}_txcohort-{tolower(faci)}_{yr}-{mo}.xlsx"))
}

generate_tx_cohort(yr, mo, 'SJD')
